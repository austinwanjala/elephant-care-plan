-- Update handle_new_user to handle constraints safely
...
BEGIN
    ...
    IF _role = 'member' THEN
        ...
        -- id_number is UNIQUE NOT NULL, so we must generate a unique placeholder if missing
        IF _id_number IS NULL OR _id_number = '' THEN
             -- Use a simplified random string or the user ID segment to ensure uniqueness
             _id_number := 'PENDING-' || SUBSTRING(NEW.id::TEXT, 1, 8);
        END IF;

        -- phone is NOT NULL but not UNIQUE in older schema? 
        -- Checking schema: phone TEXT NOT NULL.
...
EXCEPTION WHEN OTHERS THEN
    -- Log error for debugging (if system_logs table exists and we want to use it, or just raise)
    -- RAISE EXCEPTION 'Error in handle_new_user: %', SQLERRM;
    -- For now, re-raise to fail the transaction so the user sees the error
    RAISE;
END;
$$;
