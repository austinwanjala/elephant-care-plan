-- Migration: 20260311000005_update_finalize_rpc_stages.sql
...
BEGIN
    ...
    -- If a stage was performed in this visit, mark it as "moved to next" or keep as "completed" if it was final
    UPDATE public.service_stages
    SET 
        status = CASE WHEN current_stage >= total_stages THEN 'completed' ELSE 'in_progress' END,
        updated_at = now()
